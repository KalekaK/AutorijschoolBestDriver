...
class Gebruiker {
    ...
    public function getById(int $id): array|false {
        $stmt = $this->pdo->prepare("SELECT * FROM gebruiker WHERE Gebruiker_id = ?");
        $stmt->execute([$id]);
        return $stmt->fetch();
    }
    ...
    public function getByGebruikersnaam(string $gebruikersnaam): array|false {
        $stmt = $this->pdo->prepare(
            "SELECT * FROM gebruiker WHERE LOWER(Gebruikersnaam) = LOWER(?) AND Actief = 1"
        );
    ...
    public function bestaatGebruikersnaam(string $gebruikersnaam, int $excludeId = 0): bool {
        if ($excludeId > 0) {
            $stmt = $this->pdo->prepare(
                "SELECT COUNT(*) FROM gebruiker WHERE LOWER(Gebruikersnaam) = LOWER(?) AND Gebruiker_id <> ?"
            );
            $stmt->execute([$gebruikersnaam, $excludeId]);
        } else {
            $stmt = $this->pdo->prepare(
                "SELECT COUNT(*) FROM gebruiker WHERE LOWER(Gebruikersnaam) = LOWER(?)"
            );
            $stmt->execute([$gebruikersnaam]);
        }
    ...
    public function toevoegen(array $data): bool {
        $registratieDatum = trim((string)($data['registratiedatum'] ?? ''));
        if ($registratieDatum === '') {
